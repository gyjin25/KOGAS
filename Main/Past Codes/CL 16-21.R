...
df1601 <- df_1601$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 계) %>% 
  arrange(일자) %>% filter(발전연료=='석탄'|발전연료=='국내탄') %>% select(-발전연료) %>% 
...
df1602 <- df_1602$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 계) %>% 
  arrange(일자) %>% filter(발전연료=='3.석탄'|발전연료=='2.국내탄') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)

df1603 <- df_1603$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 계) %>% 
  arrange(일자) %>% filter(발전연료=='3.유연탄'|발전연료=='2.국내탄') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)
...
df1610 <- df_1610$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 계) %>% 
  arrange(일자) %>% filter(발전연료=='2.유연탄'|발전연료=='3.국내탄') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)
...
df1904 <- df_1904$Sheet1 %>% janitor::row_to_names(1) %>%
  select(일자, 발전기명, 발전연료, 합계) %>% 
  arrange(일자) %>% filter(발전연료=='석탄'|발전연료=='국내탄'|발전연료=='국내탄') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 합계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)

df1905 <- df_1905$`발전실적(1905)` %>% janitor::row_to_names(1) %>% 
  select(일자, 발전기명, 발전연료, 계) %>% 
  arrange(일자) %>% filter(발전연료=='COAL') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)
...
df1910 <- df_1910$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 합계) %>% 
  arrange(일자) %>% filter(발전연료=='COAL') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 합계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)

df1911 <- df_1911$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 합계) %>% 
  arrange(일자) %>% filter(발전연료=='COAL') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 합계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)
...
df2010 <- df_2010$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 합계) %>% 
  arrange(일자) %>% filter(발전연료=='COAL') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 합계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)

df2011 <- df_2011$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 합계) %>% 
  arrange(일자) %>% filter(발전연료=='COAL') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 합계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)
...
df2103 <- df_2103$`2021년 3월 발전량` %>% janitor::row_to_names(1) %>% 
  ...
  arrange(일자) %>% filter(발전연료2=='3.석탄'|발전연료2=='2.국내탄') %>% select(-발전연료2) %>% 
  spread(key = 발전기명, value = 합계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)
...
df_coal <- plyr::rbind.fill(df16,df17,df18,df19,df20,df21,df22) %>% 
  select(-"태안복합GT(비중앙소계)",-"태안복합ST(비중앙소계)") %>% 
  replace(is.na(.), 0)

df_coal %>% colnames

save(df_coal, file="/Users/user/Dropbox/KOGAS/R Data/df_coal_2204.RData")
