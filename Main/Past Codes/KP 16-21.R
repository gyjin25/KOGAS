...
df1601 <- df_1601$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 계) %>% 
  ...
  replace(is.na(.), 0) %>% select("광양GT#1","광양GT#2","광양GT#3","광양GT#4","광양ST#1","광양ST#2")
...
df_KP <- plyr::rbind.fill(df16,df17,df18,df19,df20,df21,df22) %>%  
  replace(is.na(.), 0)

save(df_KP, file="/Users/user/Dropbox/KOGAS/R Data/df_KP_2204.RData")
