...
df1601 <- df_1601$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 계) %>% 
  arrange(일자) %>% filter(발전연료=='기타') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0) %>% select("아산GT#1","아산GT#2","아산ST#1" )
...
df1603 <- df_1603$Sheet1 %>% janitor::row_to_names(1) %>% select(일자, 발전기명, 발전연료, 계) %>% 
  arrange(일자) %>% filter(발전연료=='8.8.기타') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0)%>% select("아산GT#1","아산GT#2","아산ST#1" )
...
df1606 <- df_1606$Sheet1 %>% select(일자, 발전기명, 발전연료, 계) %>% 
  arrange(일자) %>% filter(발전연료=='8.기타') %>% select(-발전연료) %>% 
  spread(key = 발전기명, value = 계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0) %>% select("아산GT#1","아산GT#2","아산ST#1" )
...
df1905 <- df_1905$`발전실적(1905)` %>% janitor::row_to_names(1) %>% 
  ...
  arrange(일자) %>% filter(발전연료2=='기타') %>% select(-발전연료2) %>% 
  spread(key = 발전기명, value = 계) %>% mutate_all(function(x) as.numeric(as.character(x))) %>% 
  replace(is.na(.), 0) %>% select("아산GT#1","아산GT#2","아산ST#1" )
...
df_LH <- plyr::rbind.fill(df16,df17,df18,df19,df20,df21,df22) %>%  
  replace(is.na(.), 0)

df_LH[1:1827,] <- 0 # 2021년 1월 1일부터 CHP 발전

save(df_LH, file="/Users/user/Dropbox/KOGAS/R Data/df_LH_2204.RData")
